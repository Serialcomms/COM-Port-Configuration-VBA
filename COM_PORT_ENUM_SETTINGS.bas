...
COM_PORTS(Port_Index).New_Comm_Config.DCB = COM_PORTS(Port_Index).New_DCB
COM_PORTS(Port_Index).New_Comm_Config.Size = LenB(COM_PORTS(Port_Index).New_Comm_Config)

Port_Config_Dialogue COM_PORTS(Port_Index).Name, LONG_0, COM_PORTS(Port_Index).New_Comm_Config
...
MESSAGE_BOX_TEXT = "Apply settings " & New_Settings & " to port " & COM_PORTS(Port_Index).Name & " ? "
...
If OPEN_COM_PORT(Port_Index) Then
    ...
    Apply_Result = Apply_Port_DCB(COM_PORTS(Port_Index).Handle, COM_PORTS(Port_Index).New_Comm_Config.DCB)

    If Apply_Result Then
          
        Temp_Text = " Updated" & TEXT_NEWLINES & "Old Settings = " & Old_Settings & TEXT_NEWLINES & "New Settings = " & New_Settings
   
        MESSAGE_BOX_TEXT = COM_PORTS(Port_Index).Name & Temp_Text
        MESSAGE_BOX_BUTTONS = vbInformation + vbOKOnly
        MESSAGE_BOX_RESULT = MsgBox(MESSAGE_BOX_TEXT, MESSAGE_BOX_BUTTONS, MESSAGE_BOX_TITLE)
        
        COM_PORTS(Port_Index).Settings = New_Settings
        Com_Port_Ribbon.InvalidateControl ("CP_Settings")
     
     Else
     
        DLL_Error = Err.LastDllError
        Temp_Text = COM_PORTS(Port_Index).Name & TEXT_NEWLINES & "Old Settings = " & Old_Settings & vbCrLf
     ...
     Com_Port_Close COM_PORTS(Port_Index).Handle
     
     COM_PORTS(Port_Index).Handle = LONG_0
...
If Not Open_Result Then
    ...
    MESSAGE_BOX_TEXT = "Error Opening Port " & COM_PORTS(Port_Index).Name
...
Else
    ...
    Label_Suffix = IIf(Len(COM_PORTS(Com_Port_Index).Settings) = LONG_0, "Settings", COM_PORTS(Com_Port_Index).Settings)
    
    returnedVal = COM_PORTS(Com_Port_Index).Name & vbCrLf & Label_Suffix
