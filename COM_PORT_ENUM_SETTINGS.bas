...
Public Com_Port_Index As Long
Public Com_Port_Names() As String
Public Com_Port_Labels() As String
Public Com_Port_Settings() As String
...
Private Const TEXT_SPACES As String = "  "
...
Private Port_DCB As DEVICE_CONTROL_BLOCK
Private Port_Name As String
Private Port_Handle As LongPtr
...
Public Function SET_PORT_DIALOGUE(Optional Port_Name As String) As String

Dim DLL_Error As Long
Dim New_Port_Settings As String

Dim Port_Comm_Config As COMM_CONFIG
Dim Port_DCB As DEVICE_CONTROL_BLOCK

Port_Comm_Config.DCB = Port_DCB
Port_Comm_Config.Size = LenB(Port_Comm_Config)

If Len(Port_Name) > LONG_3 Then
     
    Port_Config_Dialogue Port_Name, LONG_0, Port_Comm_Config
...
Else ' port name invalid

    New_Port_Settings = "PORT-NAME-ERROR"
...
Dim MESSAGE_BOX_BUTTONS As Long

MESSAGE_BOX_TEXT = "Apply settings " & New_Settings & " to port " & Port_Name & " ? "
MESSAGE_BOX_TITLE = Com_Port_Labels(Com_Port_Index) & " Settings"
...
If OPEN_COM_PORT(Port_Name) Then
    ...
    Query_Port_DCB Port_Handle, Settings_DCB
     ...
     With Settings_DCB
        ...
        Com_Port_Settings(Com_Port_Index) = Old_Settings
        
        Com_Port_Ribbon.InvalidateControl ("CP_Settings")
        
     End With
     
     If Apply_Port_DCB(Port_Handle, Settings_DCB) Then
     
      MESSAGE_BOX_TEXT = Port_Name & " Updated" & vbCrLf & vbCrLf & "Old Settings = " & Old_Settings & vbCrLf & "New Settings = " & New_Settings
      MESSAGE_BOX_TITLE = Com_Port_Labels(Com_Port_Index) & " Settings " & New_Settings
      MESSAGE_BOX_BUTTONS = vbInformation + vbOKOnly
      MESSAGE_BOX_RESULT = MsgBox(MESSAGE_BOX_TEXT, MESSAGE_BOX_BUTTONS, MESSAGE_BOX_TITLE)
      
      Com_Port_Settings(Com_Port_Index) = New_Settings
     ...
     Else
      ...
      MESSAGE_BOX_TITLE = Com_Port_Labels(Com_Port_Index) & " Error"
...
ReDim Com_Port_Names(LONG_0 To Com_Port_Count)
ReDim Com_Port_Labels(LONG_0 To Com_Port_Count)
ReDim Com_Port_Settings(LONG_0 To Com_Port_Count)

Com_Port_Labels(LONG_0) = TEXT_NO_COM_PORTS
    
If Com_Port_Count > LONG_0 Then

    Com_Port_Labels(LONG_0) = TEXT_NO_COM_PORT
    
    For Port_Ordinal = LONG_1 To Com_Port_Count
    
        Port_Number = Temp_Port_Numbers(Port_Ordinal)
    
        Com_Port_Names(Port_Ordinal) = TEXT_COM & Port_Number
        
        Com_Port_Labels(Port_Ordinal) = TEXT_COM_PORT & Port_Number
        
        Com_Port_Settings(Port_Ordinal) = TEXT_SETTINGS
...
Read_Ribbon_Combo = IIf(Com_Port_Index = LONG_0, vbNullString, Com_Port_Names(Com_Port_Index))
...
Private Sub GetPortCount(Control As IRibbonControl, ByRef Combo_Entries)                    ' Ribbon Callback for CP_Selector getPortCount

Combo_Entries = LONG_1 + Query_Com_Ports

Com_Port_Ribbon.InvalidateControl ("PS_Settings")
...
PortLabel = Com_Port_Labels(Index)

End Sub

Private Sub GetPortSelection(Control As IRibbonControl, id As String, PortSelection As Long) ' Ribbon Callback for CP_Selector onChange

Debug.Print "GetPortSelection, ID = " & id & " , Selection = " & PortSelection

Com_Port_Index = PortSelection
...
returnedVal = IIf(Com_Port_Index > LONG_0, True, False)

End Sub

Private Sub PortSettings(Control As IRibbonControl)                                         ' Ribbon Callback for CP_Settings onAction

SET_PORT_DIALOGUE Com_Port_Names(Com_Port_Index)
...
returnedVal = Com_Port_Names(Com_Port_Index) & vbCrLf & Com_Port_Settings(Com_Port_Index)
